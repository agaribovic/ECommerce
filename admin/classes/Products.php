...
class Products
{
	...
	public function addProduct($product_name,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/vuksanSLASTICAR/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						return ['status'=> 202, 'message'=> 'Proizvod je uspjesno dodat..!'];
					}else{
						return ['status'=> 303, 'message'=> 'Nije uspjelo pokretanje upita'];
					}

				}else{
					return ['status'=> 303, 'message'=> 'Otpremanje slike nije uspjelo'];
				}

			}else{
				return ['status'=> 303, 'message'=> 'Velika slika ,Max dozvoljena 2MB'];
			}

		}else{
			return ['status'=> 303, 'message'=> 'Nevazeci forma slike [Vazeci format : jpg, jpeg, png]'];
		}
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/vuksanSLASTICAR/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						return ['status'=> 202, 'message'=> 'Proizvod je modifikovan uspjesno..!'];
					}else{
						return ['status'=> 303, 'message'=> 'Nije uspjelo pokretanje upita'];
					}
	...
	public function editProductWithoutImage($pid,
		...
		if ($pid != null) {
			...
			return ['status'=> 303, 'message'=> 'Nevazeci proizvod id'];
		}
	...
	public function addCategory($name){
		...
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'Kategorija vec postoji'];
		}else{
			$q = $this->con->query("INSERT INTO categories (cat_title) VALUES ('$name')");
			if ($q) {
				return ['status'=> 202, 'message'=> 'Nova kategorija je uspjesno dodata'];
			}else{
				return ['status'=> 303, 'message'=> 'Nije uspjelo pokretanje upita'];
			}
		}
	...
	public function deleteProduct($pid = null){
		if ($pid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Proizvod je uklonjen iz zaliha'];
			}else{
				return ['status'=> 202, 'message'=> 'Nije uspjelo pokretanje upita'];
			}
	...
	public function deleteCategory($cid = null){
		if ($cid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Kategorija Uklonjena'];
			}else{
				return ['status'=> 202, 'message'=> 'Nije uspjelo pokretanje upita'];
			}
			
		}else{
			return ['status'=> 303, 'message'=>'Nevazeca kategorija id'];
		}
	...
	public function updateCategory($post = null){
		...
		if (!empty($cat_id) && !empty($e_cat_title)) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Kategorija je azurirana'];
			}else{
				return ['status'=> 202, 'message'=> 'Nije uspjelo pokretanje upita'];
			}
	...
	public function addBrand($name){
		...
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'Sastojak vec postoji'];
		}else{
			$q = $this->con->query("INSERT INTO brands (brand_title) VALUES ('$name')");
			if ($q) {
				return ['status'=> 202, 'message'=> 'Novi sastojak je uspjesno dodat'];
			}else{
				return ['status'=> 303, 'message'=> 'Nije uspjelo pokretanje upita'];
			}
		}
	...
	public function deleteBrand($bid = null){
		if ($bid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Sastojak uklonjen'];
			}else{
				return ['status'=> 202, 'message'=> 'Nije uspjelo pokretanje upita'];
			}
			
		}else{
			return ['status'=> 303, 'message'=>'Nevazeci sastojak id'];
		}
	...
	public function updateBrand($post = null){
		...
		if (!empty($brand_id) && !empty($e_brand_title)) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Sastojak Azuriran'];
			}else{
				return ['status'=> 202, 'message'=> 'Nije uspjelo pokretanje upita'];
			}
...
if (isset($_POST['add_product'])) {
	...
	&& !empty($_FILES['product_image']['name'])) {
		...
		*/
		...
		echo json_encode(['status'=> 303, 'message'=> 'Prazna polja']);
		exit();
	}
...
if (isset($_POST['add_category'])) {
	if (isset($_SESSION['admin_id'])) {
		...
		echo json_encode(['status'=> 303, 'message'=> 'Greska Sesije']);
	}
}
...
if (isset($_POST['DELETE_PRODUCT'])) {
	...
	if (isset($_SESSION['admin_id'])) {
		if(!empty($_POST['pid'])){
			...
			echo json_encode(['status'=> 303, 'message'=> 'Nevazeci proizvod id']);
			exit();
		}
	}else{
		echo json_encode(['status'=> 303, 'message'=> 'Nevazeca sesija']);
	}
...
if (isset($_POST['DELETE_CATEGORY'])) {
	if (!empty($_POST['cid'])) {
		...
		echo json_encode(['status'=> 303, 'message'=> 'Nevazeci detalji']);
		exit();
	}
}
